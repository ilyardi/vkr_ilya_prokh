...
Rpush.reflect do |on|
  ...
  on.notification_failed do |notification|
    puts "[RPUSH#notification_failed] #{notification.inspect}"
  end
  ...
  on.fcm_canonical_id do |old_id, canonical_id|
    puts "[RPUSH#gcm_canonical_id] OLD: #{old_id}\nNEW: #{canonical_id}"
  end

  # Called when the FCM returns a failure that indicates an invalid device token.
  # You will need to delete the device_token from your records.
  on.fcm_invalid_device_token do |app, error, device_token|
    msg = "[RPUSH#on.gcm_invalid_registration_id] 1/2 #{error.inspect}, #{device_token}"

    if pd = PhoneDevice.find_by(device_token: device_token)
      msg += "\n[RPUSH#on.gcm_invalid_registration_id] 2/2 #{pd.attributes.inspect}"
      pd.destroy
    end
    puts msg
